...
s1_sequences = [word_tokenize(i) for i in x_train[:,0]]
s2_sequences = [word_tokenize(i) for i in x_train[:,1]]
...
classifier = LogisticRegression().fit(similarity, y_train)
...
s1_sequences = [word_tokenize(i) for i in x_test[:,0]]
s2_sequences = [word_tokenize(i) for i in x_test[:,1]]
